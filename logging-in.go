...
func openLogFiles(directory string, clientIpAddress string) (*os.File, *os.File) {
    ...
    baseFileName := fmt.Sprintf("%s%c%s_%s", directory, os.PathSeparator, strings.Replace(strings.Split(clientIpAddress, ":")[0], ".", "-", -1), time.Now().UTC().Format("2006-01-02_15-04-05_000"))
